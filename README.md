...
For important branches inside the loop, try using [[likely]] or [[unlikely]] and measure any performance changes.
...
Run the code extensively, including all tests, to generate profiling data

Recompile with: g++ -O3 -std=c++26 -fprofile-use main.cc

Give this build to the Instrumenter (Optimization option E)
...
Generate report: perf report --stdio > perf_report.txt

Review perf_report.txt to see call relationships and time spent in each function

Write up interesting findings and send them to the Instrumenter
...
Rerun the timings and hand them to the Instrumenter (Optimization options F and G)
